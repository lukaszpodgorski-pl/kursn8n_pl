...
Materiał to **10 modułów** (od fundamentów po wbudowanego asystenta n8n) plus flagowy poradnik porównania hostingów n8n, który jest częścią Modułu 1. Menu boczne prowadzi przez cały program w kolejności nauki, więc można iść po kolei albo wskakiwać w moduł, który akurat jest potrzebny.
...
├── modul-0-fundamenty.mdx … modul-9-n8n-assistant.mdx
├── regulamin.md  prywatnosc.md
└── porownanie-n8n-hostingow.mdx   # flagowy poradnik (część Modułu 1, w przygotowaniu)
src/config/modules.ts           # rejestr 10 modułów - jedyne źródło prawdy dla sidebara,
